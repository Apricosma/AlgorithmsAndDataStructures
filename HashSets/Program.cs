...
string test = "Hello idk what to put here";

string[] words = test.Split(' ');

Dictionary<char, List<string>> wordCollections = new Dictionary<char, List<string>>();

foreach(string word in words)
{
    char firstChar = word.ToLower().First();

    if (!wordCollections.ContainsKey(firstChar))
    {
        wordCollections.Add(firstChar, new List<string>());
    }
    wordCollections[firstChar].Add(word);
}

foreach (KeyValuePair<char, List<String>> pair in wordCollections) 
{
    
    List<string> wordList = pair.Value;
    StringBuilder sb = new StringBuilder( wordList.Count );
